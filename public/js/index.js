...
const all_e = selector => document.querySelectorAll(selector);
let date = new Date();
...
const friendSugetion = () => {
  ...
  axios.get("/api/user/", window.AUTH_HEADERS).then(
    res => {
      if (res.status === 200) {
        // console.log(res.data);
        // change to the dom element
        ...
        for (const i in res.data.data) {
          ...
          let button = "";
          if (user._id !== window.PROFILE._id) {
            button = `<button onclick="return invite(this, '${user._id}');">Invite</button>`;
          } else {
            button = "";
          }
          e("#friendListInvite").innerHTML += `
            <div class="friend_item animated" id="user${user._id}">
...
const friensInviteHide = () => {
  setTimeout(() => {
    for (const key in window.PROFILE.friends) {
      let = item = "#user" + window.PROFILE.friends[key].friend + " button";
...
const profileShow = (type = "me", id = 00) => {
  ...
  e("#profile-label-username").innerHTML = window.PROFILE.username;
  e("#profile-label-names").innerHTML = window.PROFILE.full_names;
  e("#profile-label-email").innerHTML = window.PROFILE.email;
  e("#profile-label-country").innerHTML = window.PROFILE.country;
};
...
const openSlide = () => {
  ...
  console.log(
    "[Courrent slide]: " +
      window.postSlide.slide.curent +
      "/" +
      window.postSlide.slide.max
  );
  console.log(
    "[Courrent user]: " +
      window.postSlide.userPost.curent +
      "/" +
      window.postSlide.userPost.max
  );
...
const chattList = () => {
  ...
  // request the users
  axios.get(`/api/user/${window.PROFILE._id}`, window.AUTH_HEADERS).then(
    res => {
      loading(false);

      if (JSON.stringify(res.data.data) !== JSON.stringify(window.PROFILE)) {
...
const displayMY_FRIENDS_maps = friends => {
  console.log("users: ", window.ALL_USERS);
  console.log("friends: ", friends);
  loading(false);
  // list of friends
  e("#friendsListContainner").innerHTML = "";

  let html = "";

  for (var x in friends) {
    const friend = friends[x];
    let user = window.ALL_USERS.find(us => us._id == friend.friend);
    // friendsListContainner
    html += `
          <div class="friend_item friend_item_chats animated" id="ddd${friend.friend}" onclick="return openChatt(this, '${friend.friend}', '${friend.chatt_id}');">
...
//DISPLAY ALL THE LIST OF THE USERS
const displayMY_FRIENDS = friends => {
  // get all the users
  // LOAD ALL USERS
  if (window.ALL_USERS === null) {
    axios.get("/api/user/", window.AUTH_HEADERS).then(
      res => {
        window.ALL_USERS = res.data.data;
        displayMY_FRIENDS_maps(friends);
      },
      err => {
        // console.log(err);
      }
...
const openChatt = (el, friend, group) => {
  if (window.SELECTED_UID !== friend) {
    ...
    let userSelected = window.ALL_USERS.find(ele => ele._id === friend);
    ...
    setTimeout(() => {
      e("#chattingContainnerMessages").scrollTop = objDiv.scrollHeight;
    }, 100);
    ...
    //   // axios
    //   //   .get(
    //   //     "https://african-chat-app.firebaseio.com/chats/" + group + "/chats.json"
    //   //   )
    //   //   .then(response => {
    ...
    axios
      .get(`/api/message/${window.PROFILE._id}/${friend}`, window.AUTH_HEADERS)
      .then(
        res => {
          loading(false);
          for (const x in res.data.data) {
            writeMessageDOM(res.data.data[x]);
          }
        },
        err => {
          console.log(err);
...
const writeMessageDOM = message => {
  let = html = "";
  let chatBy = "";
  if (message.from === window.PROFILE._id) {
    chatBy = "me";
  } else {
    chatBy = "friend";
  }
  ...
  setTimeout(() => {
    e("#chattingContainnerMessages").scrollTop = e(
      "#chattingContainnerMessages"
    ).scrollHeight;
  }, 100);
};
...
const invite = (el, code) => {
  ...
  //   console.log("id: ", code);
  let friendRequest = {
    userId: window.PROFILE._id,
    friendId: code
  };
  axios.post("/api/user/invite", friendRequest, window.AUTH_HEADERS).then(
    res => {
      loading(true);
      if (res.status === 200) {
        switchClass(el.parentElement, "x", "zoomOut", true);
...
const sendMessage = (msg, by) => {
  ...
  setTimeout(() => {
    e("#chattingContainnerMessages").lastElementChild.classList.remove(
      "zoomInUp"
    );
  }, 3000);
  ...
  let api_msg = {
    from: window.PROFILE._id,
    to: window.SELECTED_UID,
    message: msg
  };

  // PUSH THE MESSAGE TO THE SERVER
  loading(true);
  axios.post("/api/message", api_msg, window.AUTH_HEADERS).then(response => {
    console.log(response);

    // // handle success
    message.key = response.data.from;
...
const chattBoot = message => {
  setTimeout(() => {
    ...
    if (message == "group list") {
      replly =
        "Group Menber: <br> <b>218004581</b> Janvier <br>  <b>218004581</b> Janvier <br> <b>218004581</b> Janvier <br>  <b>218004581</b> Janvier <br>";
      ...
      replly =
        "Thank you for contacting Me! <br> i will reply you when i get back online!";
    }

    // sending message
    if (replly !== "") {
      sendMessage(replly, "friend");
    }
  }, 3000);
};
...
const usersGroups = () => {
  let foundGroups = [];
  let firends = window.PROFILE.friends;

  for (const key in firends) {
    foundGroups.push(firends[key].chatt_id);
  }
  return foundGroups;
};
  ...
  if (sessionStorage.getItem("uid") === "") {
    ...
    loadMainData(data => {
      ...
      window.socket.on("connect", function() {
        ...
        // console.log("friends ==>: ", usersGroups());

        let joinData = {
          uid: window.PROFILE._id,
          groups: usersGroups()
        };
...
const activeChatDOM = (id, type) => {
  if (type == "active") {
    e(
      `#friendsListContainner #ddd${id} .thumbnail .active-icon`
    ).style.display = "block";
  } else {
    e(
      `#friendsListContainner #ddd${id} .thumbnail .active-icon`
  ...
  console.log(e(`#friendsListContainner #ddd${id} .thumbnail .active-icon`));
  ...
  window.socket.on("newMessage", function(message) {
    ...
    if (message.group === window.SELECTED_GROUP && message.by !== window.UID) {
      ...
      if (message.by !== undefined) {
        alertMesage({
          user: window.ALL_USERS.find(userd => userd._id === message.by)
            .username,
          message: message.msg,
          id: "",
          group: ""
