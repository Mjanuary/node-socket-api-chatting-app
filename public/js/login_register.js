...
e("#registerButton").addEventListener("click", () => {
  ...
  // PHONE NUMBER VALIDATION
    ...
    username_label.innerHTML =
      "your username must not be less than 6 characters!";
    username.style.borderColor = "red";
  }
  ...
  // PASSWORD VALIDATION
    ...
    password_label.innerHTML =
      "Your password must not be greater than 20 characters!";
    password.style.borderColor = "red";
  }
  ...
  // PHONE NUMBER VALIDATION
    ...
    let credentials = {
      ...
      full_names: names.value,
      country: country.value
    };

    // console.log("sending axios data: ==========================");
    axios
      ...
      .then(
        data => {
          ...
          e("#register-form-containner").style.display = "none";
          alerts(
            "Account Created Successfully<br>Now you can login",
            "login",
            "success zoomIn"
          );
        },
        error => {
          if (error.response.status === 409) {
            alerts(
              "Username and password already exist in the system<br>Try others",
              "register"
            );
          } else {
            alerts("Something went wrong <br> Try again later", "register");
          }
        }
      );
  }
...
e("#loginButton").addEventListener("click", () => {
  ...
  if (email.value == "") {
    ...
    axios.post("/api/user/login", crd).then(
      data => {
        alerts("aUTHENTICATION SUCCESSFULLY!", "login", "success zoomIn");
...
window.onload = event => {
  ...
  if (
    ...
    window.location.replace("/chat");
    // console.log("REDIRECT TO THE USER:");
  }
